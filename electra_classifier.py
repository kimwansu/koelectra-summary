...
test_dataset = []
for doc_id in tqdm(test_doc_ids):
    sents = j_doc[doc_id]['sents']
    encoded_sents = []
    max_in_sent = 0
    minibatch = []
    ...
    test_dataset.append([input_ids_batch, attention_batch, y_answer_batch])
...
for i in range(n_epochs):
    ...
    for iid_batch, am_batch, y_batch in tqdm(train_dataset):
        ...
        batches += 1
        if batches % 100 == 0:
            print('\nbatch loss: ', total_loss, 'acc: ', correct.float() / total)
    
    loss_list.append(total_loss)
    acc_list.append(correct.float() / total)
    print(f'Epoch {i+1} : train_loss: {total_loss}, acc: {correct.float() / total}')
    torch.save(model.state_dict(), f'model_electra_c_epoch{i+1}.pt')


#torch.save(model.state_dict(), 'model_electra0.pt')
#"""

model.load_state_dict(torch.load('model_electra_b_epoch3.pt'))
...
with torch.no_grad():
    ...
    for iid_batch, am_batch, y_batch in tqdm(test_dataset):
        y_batch = torch.LongTensor(y_batch).to(device)
        #y_pred = model(iid_batch.to(device), attention_mask=am_batch.to(device))[0]
        output = model(iid_batch.to(device), attention_mask=am_batch.to(device))
        y_pred = output[0]
